import React from "react";
import Header from "../components/Header";
import "../css/About.css";
import data from "../data.json";

function About() {
  const myjson = JSON.stringify(data, null, 4);
  console.log(myjson);

  return (
    <div className='about'>
      <Header />
      <div className='about_infocard'>
        <div className='about_center'>
          <h1>Application Documentation</h1>
          <div className='obj'>
            <div>
              <p className='heading'>Objective</p>
              <p>
                Internship selection task provided by the SIMMI Foundation.
                Simmi Foundation envisions to develop a society based on
                legitimate rights, equity, justice, honesty, social sensitivity
                and a culture of service in which all are self-reliant.We look
                forward to alleviate poverty and enhance the dignty of poor in
                the world by addressing basic, but neglected issues of poor.
                Education is one of the means through which we can acheive our
                goals.Women Empowerment is another mode of development, as women
                are an important section of our society and their empowerment
                can bring socio-economic and political progress of the country.
              </p>
            </div>

            <img
              src='https://simmifoundation.org/images/simmilogo.png'
              alt='not available'
              style={{ height: 200, width: 200, margin: 50 }}
            />
          </div>
          <p className='heading head'>Tools used</p>
          <div className='tools'>
            <div className='tools_card'>
              <img
                src='https://camo.githubusercontent.com/edc736634dd35b0f4008e2f7db456136b9fc0e1e7a4078bb72c7352b1bdf8a7e/68747470733a2f2f776f726c64766563746f726c6f676f2e636f6d2f6c6f676f732f6373732d332e737667'
                alt='not available'
              />
            </div>
            <div className='tools_card'>
              <img
                src='https://cdn-icons-png.flaticon.com/512/732/732212.png'
                alt='not available'
              />
            </div>
            <div className='tools_card'>
              <img
                src='data:image/png;base64,'
                alt='not available'
              />
            </div>
            <div className='tools_card'>
              <img
                src='https://www.datocms-assets.com/45470/1631110818-logo-react-js.png'
                alt='not available'
              />
            </div>
          </div>
          <p className='heading head'>Json File used </p>
          <div className='jsondiv'>
            <pre>
              <code>{myjson}</code>
            </pre>
          </div>
        </div>
      </div>
    </div>
  );
}

export default About;
